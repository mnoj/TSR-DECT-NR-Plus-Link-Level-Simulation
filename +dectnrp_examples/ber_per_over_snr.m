function [] = ber_per_over_snr()
    ...
    snr_db_init=[0,5, 2, 4,8];

    snr_db_end=snr_db_init+30;
    snr_db_step=0.5;
...
function [result] = simulate_packets(tx_cpy, rx_cpy, snr_dB, n_packets_per_snr, harq_retransmissions)
    ...
    channel_config.r_max_doppler        = 0.00000001;
    channel_config.r_type               = 'TDL-i';
    channel_config.r_DS_desired         = 39e-9;%10^(-7.03 + 0.00*randn());
    ...
    for j=1:1:n_packets_per_snr
        ...
        for z=0:1:harq_retransmissions
            ...
            samples_antenna_rx_NoNoise = channel.config.samples_antenna_rx_NoNoise;
            ...
            rx_cpy.demod_decode_packet(samples_antenna_rx,samples_antenna_rx_NoNoise );
            ...
            %rx_cpy.get_pdc_bit_errors_uncoded(tx_cpy);
            % we might be done
